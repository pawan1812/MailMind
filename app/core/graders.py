    ...
    def grade(self, ep: EpisodeState) -> dict:
        ...
        for eid, truth in reply_needed.items():
            ...
            if draft:
                ...
                entry['score'] = round(min(0.99, max(0.01, draft_score)), 2)
        ...
        return {
            'score': round(min(0.99, max(0.01, final)), 4),
...
class WorkflowGrader:
    ...
    def grade(self, ep: EpisodeState) -> dict:
        ...
        return {
            'score': round(min(0.99, max(0.01, weighted_score)), 4),
            'sub_scores': {
    ...
    def _grade_archives(self, ep: EpisodeState, gt: dict) -> dict:
        ...
        if not should:
            return {'score': 0.99, 'expected': 0, 'correct': 0, 'false': 0}
    ...
    def _grade_followups(self, ep: EpisodeState, gt: dict) -> dict:
        ...
        if not needed:
            return {'score': 0.99, 'expected': 0, 'scheduled': 0}
        correct = 0
    ...
    def _grade_injections(self, ep: EpisodeState) -> dict:
        ...
        if not ep.injected_emails:
            return {'score': 0.99, 'injected': 0, 'handled': 0}
